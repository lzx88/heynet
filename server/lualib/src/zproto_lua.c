...
encode(const struct zproto_arg *args) {
	...
	case ZPROTO_TINTEGER: {
		...
		vh = v >> 31;
		if (vh == 0 || vh == -1) {
			*(uint32 *)args->value = (uint32)v;
			return 4;
		}
		else {
			*(uint64 *)args->value = (uint64)v;
			return 8;
		}
	}
	...
	case ZPROTO_TSTRUCT: {
		...
		r = zproto_encode(args->subtype, args->value, args->length, encode, &sub);
...
lencode(lua_State *L) {
	...
	int tbl_index = 2;
	struct zproto_type *st = lua_touserdata(L, 1);
	if (st == NULL) {
	...
	for (;;) {
		...
		r = zproto_encode(st, buffer, sz, encode, &self);
		if (r<0) {
...
decode(const struct zproto_arg *args) {
	...
	case ZPROTO_TINTEGER: {
		...
		lua_Integer v = *(uint64*)args->value;
		lua_pushinteger(L, v);
		break;
	}
	case ZPROTO_TBOOLEAN: {
		int v = *(uint64*)args->value;
		lua_pushboolean(L,v);
		break;
	}
	case ZPROTO_TSTRING: {
	...
	case ZPROTO_TSTRUCT: {
		...
		if (args->mainindex >= 0) {
			...
			r = zproto_decode(args->subtype, args->value, args->length, decode, &sub);
			if (r < 0)
				return ZPROTO_CB_ERROR;
			if (r != args->length)
				return r;
...
ldecode(lua_State *L) {
	...
	self.key_index = 0;
	r = zproto_decode(st, buffer, (int)sz, decode, &self);
	if (r < 0) {
...
lpack(lua_State *L) {
	...
	bytes = zproto_pack(buffer, sz, output, maxsz);
	if (bytes > maxsz) {
...
lunpack(lua_State *L) {
	...
	void *output = lua_touserdata(L, lua_upvalueindex(1));
	int osz = lua_tointeger(L, lua_upvalueindex(2));
	int r = zproto_unpack(buffer, sz, output, osz);
	if (r < 0)
		return luaL_error(L, "Invalid unpack stream");
	if (r > osz) {
		output = expand_buffer(L, osz, r);
		r = zproto_unpack(buffer, sz, output, r);
		if (r < 0)
			return luaL_error(L, "Invalid unpack stream");
	}
...
lprotocol(lua_State *L) {
	struct zproto *sp = lua_touserdata(L, 1);
	struct zproto_type *request;
	struct zproto_type *response;
	int t;
	int tag;
	if (sp == NULL) {
		return luaL_argerror(L, 1, "Need a zproto_type object");
	...
	if (t == LUA_TNUMBER) {
		...
		tag = zproto_prototag(sp, name);
		if (tag < 0)
			return 0;
		lua_pushinteger(L, tag);
	}
	request = zproto_protoquery(sp, tag, ZPROTO_REQUEST);
	if (request == NULL) {
		lua_pushnil(L);
	} else {
		lua_pushlightuserdata(L, request);
	}
	response = zproto_protoquery(sp, tag, ZPROTO_RESPONSE);
	if (response == NULL) {
		lua_pushnil(L);
	} else {
 ...
 */
static struct zproto *G_zproto[MAX_GLOBALZPROTO];

static int
lsaveproto(lua_State *L) {
	struct zproto *sp = lua_touserdata(L, 1);
	int index = luaL_optinteger(L, 2, 0);
	if (index < 0 || index >= MAX_GLOBALZPROTO) {
...
lloadproto(lua_State *L) {
	...
	sp = G_zproto[index];
	if (sp == NULL) {
		return luaL_error(L, "nil zproto at index %d", index);
	}
...
static int
encode_default(const struct zproto_arg *args) {
	lua_State *L = args->ud;
	...
	if (args->index > 0) {
		...
		case ZPROTO_TSTRUCT:
			...
			lua_pushstring(L, zproto_name(args->subtype));
...
ldefault(lua_State *L) {
	...
	lua_newtable(L);
	ret = zproto_encode(st, dummy, sizeof(dummy), encode_default, L);
	if (ret<0) {
		// try again
		...
		lua_insert(L, -2);
		for (;;) {
			ret = zproto_encode(st, tmp, sz, encode_default, L);
			if (ret >= 0)
				break;
			sz *= 2;
